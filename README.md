# Nginx mmap geo module
   ngx_geo_mod 是一个地理位置查询模块。其根据客户端IP查询出对应的省份，城市，ISP信息，写入请求头。
   
   本模块包含一个将文本格式的地理位置信息数据编译成二进制数据的编译器。模块运行时直接使用二进制数据，并且采用mmap方式加载文件，不需要任何解析过程。
...
* [编译器的使用](#编译器的使用)
    * [数据文件格式](#数据文件格式)
    * [编译数据文件](#编译数据文件)
    * [使用geot测试二进制GEO文件](#使用geot测试二进制GEO文件)
...
指定二进制的GEO数据文件路径。该数据文件使用geoc编译生成。生成方法参考[编译器的使用](#编译器的使用)
...
* x-isp *ISP信息*
* x-ip *IP*
请求头可以在nginx各种模块中使用。使用方法见[变量的使用](#变量的使用)。
...
* 如果ip_from_head开启了：
    * 先从请求头X-Real-IP中获取，如果未设置，从请求头X-Forwarded-For中获取。
